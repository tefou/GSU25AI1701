...
def MSRInitializer(Layer, ActivationGain=1):
    ...
    std = ActivationGain / math.sqrt(FanIn) if ActivationGain > 0 else 0.0
...
class DiscriminativeBasis(nn.Module):
    def __init__(self, InputChannels, OutputDimension):
        ...
        self.Basis = MSRInitializer(
            nn.Conv2d(InputChannels, InputChannels, kernel_size=4, stride=1, padding=0, groups=InputChannels, bias=False)
        )
...
class GeneratorStage(nn.Module):
    def __init__(self, InputChannels, OutputChannels, Cardinality, NumberOfBlocks, ExpansionFactor, KernelSize, VarianceScalingParameter, ResamplingFilter=None, DataType=torch.float32):
        ...
        TransitionLayer = Convolution(InputChannels, OutputChannels, KernelSize=3) if ResamplingFilter is None else UpsampleLayer(InputChannels, OutputChannels, ResamplingFilter)
...
class Generator(nn.Module):
    def __init__(self, InputChannels, WidthPerStage, CardinalityPerStage, BlocksPerStage, ExpansionFactor, KernelSize=3, ResamplingFilter=[1, 2, 1]):
        super(Generator, self).__init__()

        VarianceScalingParameter = sum(BlocksPerStage)

        MainLayers = [GeneratorStage(InputChannels, WidthPerStage[0], CardinalityPerStage[0], BlocksPerStage[0], ExpansionFactor, KernelSize, VarianceScalingParameter)]
